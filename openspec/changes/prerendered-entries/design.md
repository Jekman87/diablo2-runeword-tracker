...
The alternative, hydrating and then applying stored state in an effect after
mount, was considered and rejected for now. It trades a one-frame flicker for a
guaranteed visible flip, adds a two-phase initialisation to three stores whose
current lazy-init is deliberate ("an effect would render English and then correct
it, a visible frame of a language the player left" — the locale store's own
comment), and buys a benefit — skipping one render of a small page — that nobody
has measured wanting. Recorded as future work, conditional on measurement.

The cost accepted: React rebuilds the DOM once on mount, and a reader with stored
progress may see the empty state for a frame. On a page of this size that is
cheap, and the reader gets a first paint sooner than today's empty `#root`.
...
- **A reader with progress sees the empty state for a frame** → Accepted above.
  If it reads badly in practice, the answer is the hydration follow-up, not a
  patch that hides the prerender from readers.
...
produce markup; then wire it into `pnpm build` and add the output check; then the
Yandex tag, which is independent of all of it.

Rollback is a revert, and a partial rollback is safe: without the render pass the
documents serve exactly as they do today.
