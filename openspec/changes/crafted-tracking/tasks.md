...
- [x] 1.1 Add `--color-crafted-row`, `--color-progress-track` and `--color-toast`
      to the `@theme` block in `src/index.css`, each named for the surface it
      styles per the theme's role-naming rule
- [x] 1.2 Value them from the existing palette rather than a new hue — the tint is
      ...
      does
- [x] 1.3 Add **no** token for the progress bar's filled portion. `--color-crafted`
...
- [x] 1.4 Add **no** token for the collapsible panels. They belong to
      `remaining-panels`, and the theme spec's scenario now names them as the
      surfaces that still have none
- [x] 1.5 Add the `<progress>` pseudo-element rules to `src/index.css` —
...
- [x] 1.6 Add the new copy to `src/i18n/en.ts`: the crafted column header, the
...
- [x] 2.3 Put the version in the **key**, not in the payload, so a future v2 writes
      elsewhere and leaves v1 recoverable instead of destroying it in place. The
      payload is a bare JSON array of names
- [x] 2.4 Validate what comes back with a `zod` schema, not a cast. `localStorage`
...
- [x] 2.5 Return two things from the load: the names present in the dataset, and
      the remainder. Collapse duplicates through a `Set` on the way
- [x] 2.6 Write the union of the crafted set and the preserved remainder, **sorted**,
      so the same marks always produce the same stored bytes regardless of the order
      they were made in
- [x] 2.7 Guard every storage access inside this module. A disabled or private-mode
      store throws on read and on write, and no component may have to know that
- [x] 2.8 Test loading and saving a round trip; malformed JSON; valid JSON of the
...
- [x] 2.9 Test that the stored value is byte-identical for the same set marked in
      two different sequences

## 3. Crafted state hook

- [x] 3.1 Create `src/crafted/useCraftedRunewords.ts` owning the crafted
      `Set<string>`, the preserved unknown names, the toggle and the pending undo
- [x] 3.2 Initialise from `loadCrafted()` once, in a lazy `useState` initialiser
      rather than an effect, so the first render already has the player's progress
      and no empty frame renders before it
- [x] 3.3 Copy-on-write on every toggle — a new `Set` from the old. Mutating in
      place does not change identity and React will not re-render
- [x] 3.4 Call `saveCrafted()` from the toggle, **never from an effect**. An effect
      fires on mount, which would write an empty set over the data that failed to
      parse and take the evidence with it
- [x] 3.5 Key the set by canonical runeword name. Not an index — the dataset is
      generated, and an index points at a different runeword after a regeneration
- [x] 3.6 Record the pending undo as the runeword, the direction, and the DOM node
      to return focus to. One record, replaced by the next toggle — no history stack
- [x] 3.7 Test the toggle both ways, that undo reverses exactly the last toggle,
...
- [x] 3.8 Test that mounting the hook performs no write

## 4. The toggle control

- [x] 4.1 Create `src/components/CraftedToggle.tsx` — a real `<button>` with
      `aria-pressed`, drawn as an empty versus a filled socket
- [x] 4.2 Build it as a button and not a checkbox. `IDEAS.md` settles this, and a
...
- [x] 4.3 Give it an accessible name naming both the runeword and what the next
      activation does, from the layer, so it is unambiguous reached out of row
      context
- [x] 4.4 Make the filled versus empty rendering carry the state, not the colour.
      A player with a red-green deficiency reads the socket
- [x] 4.5 Compose the two states with `clsx` or a `cva` variant rather than a
      hand-rolled conditional string, per the styling rules
- [x] 4.6 Test both pressed states, the accessible name in each direction, and
      activation by Space and by Enter

## 5. Row integration

- [x] 5.1 Add the toggle cell as the row's first `<td>` in
      `src/components/RunewordRow.tsx`, and its `<th scope="col">` as the table's
      first header in `src/components/RunewordTable.tsx`
- [x] 5.2 Thread the crafted set and the toggle from `App` through `RunewordTable`
      to the rows as props. No context and no store library — one value, two
      levels, one tree
- [x] 5.3 Add the row-level `onClick` for the enlarged pointer target. Do **not**
...
- [x] 5.4 Ignore clicks whose target sits inside a nested interactive element,
      ...
      being revisited
- [x] 5.5 Confirm the name button still opens the detail view and does **not** mark
      the runeword crafted. This is the collision `runeword-table` recorded when it
      made the name a button
- [x] 5.6 Ignore a click that ends a text selection, checked with `getSelection()`.
      Dragging across a row to copy it should not toggle it
- [x] 5.7 Apply the crafted row's tint and left accent border from the tokens in
      group 1
- [x] 5.8 Hold a ref to the row's own toggle button and pass it on both paths, so
      a row-level click and a control activation record the same focus target
- [x] 5.9 Test that clicking the row toggles, that clicking the name does not, that
      clicking the control toggles exactly once rather than twice, and that no row
      is focusable by Tab
- [x] 5.10 Test that the table still exposes 99 rows with column headers after the
      column is added, so `runeword-table`'s semantics requirement still holds

## 6. Progress indicator

- [x] 6.1 Create `src/components/CraftedProgress.tsx` using a native `<progress>`,
...
- [x] 6.2 Take the maximum from the dataset's own length. Not a literal `99`, which
      goes stale on the patch that adds a runeword
- [x] 6.3 Take the maximum from the dataset **directly**, never from a count of
      ...
      accident
- [x] 6.4 State the count in text beside the bar, from the layer. A bar alone does
      not say `37`
- [x] 6.5 Test that the value tracks toggles immediately, that the maximum is 99,
...
- [x] 7.1 Create `src/components/UndoToast.tsx` — one notice, describing the most
      recent toggle, with an undo action
- [x] 7.2 Render the `role="status"` container from first render and put the notice
      inside it. A live region injected at the moment its content appears is
      unreliably announced
- [x] 7.3 Use `status` rather than `alert`. A confirmation of the player's own
      action should not interrupt what a screen reader is reading
- [x] 7.4 Dismiss after six seconds, and **do not dismiss while focus is inside
...
- [x] 7.5 Do not move focus when the notice appears
- [x] 7.6 On undo, reverse that one toggle, dismiss the notice, and return focus to
      the toggle it reverted using the node recorded in task 3.6
- [x] 7.7 Replace the notice on a further toggle rather than stacking a second one
- [x] 7.8 Test appearance, the undo reversing exactly one toggle, replacement by a
...
- [x] 7.9 Do not build a history stack, a redo, or a queue. The notice is the
...
- [x] 8.1 Wire the hook in `src/App.tsx` and render the progress indicator below
      the divider and the notice at the end of the page
- [x] 8.2 Build **no** site header. The patch line and the Help, Feedback and
...
- [x] 8.3 Update `src/App.test.tsx` for the progress indicator and the live region
- [x] 8.4 Mark the `crafted-tracking` row done in `IDEAS.md`, and record there that
...
- [x] 8.5 Record in `IDEAS.md` that `csv-import-export` must write through
...
- [x] 9.3 Confirm no component carries a literal colour value, and that the three
      new tokens are the only ones added
- [x] 9.4 Confirm no module outside `src/crafted/storage.ts` names the storage key
...
- [x] 9.11 Commit as `feat(tracking): add crafted state, persistence and progress`
