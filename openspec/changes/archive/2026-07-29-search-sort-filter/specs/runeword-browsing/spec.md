...
columns a reader is searching. Matching SHALL ignore case and SHALL ignore
surrounding whitespace in the query, and an empty query SHALL narrow nothing.
...
- **WHEN** `lea` is typed into the search field
...
- **WHEN** `SPIRIT`, `spirit` and `Spirit` are each typed
- **THEN** the same rows are presented in all three cases
...
Ascending on crafted state SHALL present un-crafted runewords first, because the
reason to order a tracker by crafted state is to see what is left.
...
#### Scenario: Crafted ascending shows what is left first

- **WHEN** the `Crafted` header is activated with some runewords marked
- **THEN** the un-crafted runewords are presented before the crafted ones
...
A sort SHALL be deterministic: the same dataset, the same crafted set and the same
chosen column SHALL always produce the same sequence of rows, so that a row's
position is a property of the data rather than of the sort implementation or of
which rows the filters happened to leave behind.
...
The interface SHALL offer a filter over equipment slot with five choices — every
slot, helm, weapon, shield and body armour — presenting all of them by default. A
runeword SHALL match a slot when any of its base item categories belongs to that
slot, so that a runeword which can go into more than one slot is presented under
each of them.
...
#### Scenario: A runeword spanning two slots appears under both

- **WHEN** the weapon slot is selected and then the body armour slot
- **THEN** `Fortitude` is presented in both cases, because it names `Weapons` and
  `Body Armors`
...
names to exactly one of the four equipment slots, and the slot filter SHALL read
...
- **THEN** each resolves to exactly one slot
...
#### Scenario: An off-hand category is a shield

- **WHEN** the slot of `Grimoire` is looked up
- **THEN** it is the shield slot, which is the slot every one of its seven
  runewords already occupies through `Shields`

#### Scenario: The general weapon categories are weapons

- **WHEN** the slots of `Weapons`, `Melee Weapons` and `Missile Weapons` are
  looked up
- **THEN** each is the weapon slot
...
- **WHEN** the slot is set to shield, the crafted filter to the remaining ones,
  and `spirit` is typed
