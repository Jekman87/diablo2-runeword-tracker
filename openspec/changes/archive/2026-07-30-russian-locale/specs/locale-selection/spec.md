...
Where no language preference is stored, the interface SHALL open in English,
whatever language the browser reports — there SHALL be no browser-language
detection. Russian is only ever entered through the switch, so the first paint
is the same in every environment. A first visit SHALL NOT write a preference.

#### Scenario: A first visit opens in English

- **WHEN** the application loads with no stored preference
...
applies the right pronunciation rules. The static document SHALL declare
English as its pre-script default.
