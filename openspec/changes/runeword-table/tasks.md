...
- [ ] 1.1 Add `src/i18n/en.ts` holding a nested object of English strings, and
...
- [ ] 1.2 Export a `useStrings()` hook that returns the active locale's record.
      ...
      copy
- [ ] 1.3 Add **no** provider, context, locale switch, persisted preference,
...
- [ ] 1.4 Populate keys only as the components in groups 3 to 7 need them. A key
      with no call site is copy nobody has decided on
- [ ] 1.5 Keep dataset values out of the layer — runeword names, rune names, item
...
- [ ] 1.6 Test that the English record satisfies the `Strings` type and that a
...
- [ ] 2.1 Add `--color-row-line`, `--color-row-hover`, `--color-panel` and
      `--color-backdrop` to the `@theme` block in `src/index.css`. Name each for
      the surface it styles, per the theme's role-naming rule
- [ ] 2.2 Take each value from the existing palette family — muted for the line and
...
- [ ] 2.3 Add the default comparator: required level ascending, name as tiebreak.
      Keep it a plain exported function in its own module, not a component helper
- [ ] 2.4 Apply it once at module scope over the static dataset. Sorting inside a
      component would re-sort 99 records per render, and `useMemo` around a
      constant is ceremony
- [ ] 2.5 Test the comparator over the whole dataset: levels never descend, ties
      are in name order, and the result is 99 records with none lost or duplicated
- [ ] 2.6 Verify the tiebreak test bites — remove the name fallback and confirm the
      test fails. Ten runewords share one level, so it should. Then restore

## 3. Property line rendering

- [ ] 3.1 Split each line with `String.prototype.split` on a **capturing group**,
...
- [ ] 3.2 Do **not** implement this with `replace`, with injected markup, or by
...
- [ ] 3.3 Match an optional sign, digits, an optional decimal part, an optional
...
- [ ] 3.4 Render matched fragments in `property-value` and the rest in `property`
- [ ] 3.5 Test the round-trip across **all 975 property lines** in the dataset, not
      a sample: concatenating the rendered fragments equals the source line exactly
- [ ] 3.6 Assert the named cases explicitly: `+50% Enhanced Defense`;
...
- [ ] 3.7 Assert that a line with no digit renders as one unemphasised fragment.
      Sixty-six lines are in that case
- [ ] 3.8 Verify the round-trip test bites: make the pattern consume its match,
      confirm the test fails and names the offending line, then restore
- [ ] 3.9 If the derivation ever cannot satisfy the round-trip, drop the emphasis
...
- [ ] 4.1 Render a badge per field the runeword carries: the patch value, `L` for
      ladder-only, `Note!` where a note exists. A runeword carrying none renders
      nothing — no placeholder, no empty slot
- [ ] 4.2 Give each badge its full meaning as its accessible name, not the letter
      drawn on screen, and a `title` for pointer users
- [ ] 4.3 Use `class-variance-authority` for the three badge variants rather than a
      hand-rolled conditional class string, and the `ladder` and `patch` tokens the
      theme already declares
- [ ] 4.4 Read no availability field anywhere but here. No ordering, counting or
      ...
      violate it
- [ ] 4.5 Test that `Mosaic` renders all three, that a pre-2.4 non-ladder runeword
      renders none, and that exactly 9 rows carry the ladder marker

## 5. Row and table

- [ ] 5.1 Build the table as a real `<table>` with `<th scope="col">` headers, so
      the 99 rows are navigable by row and column and `search-sort-filter` inherits
      real headers to make interactive
- [ ] 5.2 Add a visually hidden `<caption>` from the strings layer
- [ ] 5.3 Render the four read-only columns — name, runes, item categories,
      required level — and **no crafted-state column**. The toggle and its column
      are `crafted-tracking`
- [ ] 5.4 Render the rune sequence in dataset order with repeats intact, one icon
      per socketed rune, each carrying its canonical name as its accessible label.
      `Infinity` shows four icons with `Ber` twice
- [ ] 5.5 Set a smaller `--rune-size` at the row's use site. Do not restate a
...
- [ ] 5.6 Join item categories as a list and add the restriction **in parentheses**
...
- [ ] 5.7 Render no parentheses at all — not an empty pair — for a runeword with no
      restriction
- [ ] 5.8 Do not link item categories to their reference URL. Four of the twenty
      have none, so linking is partial by construction, and 99 rows of external
      links is noise in a column readers scan
- [ ] 5.9 Separate rows with a hairline and add a hover state. No zebra striping —
      alternating backgrounds on pure black either vanish or turn muddy, and the
      reference does not stripe
- [ ] 5.10 Add no sticky header. It earns its place once the headers are sort
      controls, so it belongs to the change that makes them interactive
- [ ] 5.11 Do **not** adopt `rune-bg2.gif`, the question `d2-theme` left for this
      ...
      they can be clicked
- [ ] 5.12 Test that all 99 rows render, that every runeword appears exactly once,
      and that the table exposes 99 rows and its column headers by role

## 6. Responsive collapse of the runes column

- [ ] 6.1 Render the rune sequence twice per row: once as the runes `<td>`, hidden
      below `md`, and once inside the name cell, hidden at `md` and above. Collapse
      the column's `<th>` with it
- [ ] 6.2 Drive it from the stylesheet with Tailwind's responsive utilities. No
      ...
      transition to flash
- [ ] 6.3 Hide with `display: none` rather than visual hiding, so the inactive copy
      leaves the accessibility tree too and exactly one sequence is perceivable at
      any width
- [ ] 6.4 Comment the duplication at the use site. CSS cannot move content between
...
- [ ] 6.5 Accept the cost explicitly: up to 686 icon spans instead of 343, half of
      them not rendered

## 7. Detail view

- [ ] 7.1 Make the runeword's name a real `<button>` inside its cell, not a click
      handler on the cell, so it is focusable and operable by Space and Enter
- [ ] 7.2 Use **one** native `<dialog>` for the whole table, driven by state
...
- [ ] 7.3 Open with `showModal()`, which supplies the focus trap, Escape to close,
      `::backdrop`, inert content behind and focus restored to the invoking button.
      Do not hand-roll any of them
- [ ] 7.4 Add backdrop click-to-dismiss against the dialog's own bounding box — the
      one dismissal `<dialog>` does not provide
- [ ] 7.5 Present the name, the rune sequence with each rune's name as a label at
      full icon size, the socket count, the item categories with any restriction,
      and every property line in dataset order
- [ ] 7.6 Derive the socket count from `runes.length` at the point of display.
      There is no socket field to read and none may be added — `Infinity` shows 4
- [ ] 7.7 Restate patch, ladder status and the note in **full words**, so every
      ...
      a hover
- [ ] 7.8 Test opening, Escape dismissal, focus returning to the invoking name, and
      that activating a second name replaces the first rather than showing both
- [ ] 7.9 Test that `Fortitude` shows all 26 property lines in order, and that no
      detail markup is in the document while nothing is open
- [ ] 7.10 Do not assert backdrop geometry or the visual dim in jsdom — it
      implements `showModal` but not layout. Those belong to the browser check in
      task 9.5

## 8. Page shell and documentation

- [ ] 8.1 Replace the 33-rune grid in `src/App.tsx` with the table, keeping the
...
- [ ] 8.2 Update `src/App.test.tsx` accordingly
- [ ] 8.3 Build **no** site header. The patch line and the Help, Feedback and
...
- [ ] 8.4 Record in `IDEAS.md` that the header has no change that builds it, and
      mark the `runeword-table` row done
- [ ] 8.5 Point `docs/CODE_RULES.md` at the i18n layer's actual location — its
      components rule already forbids display literals and refers to a layer that
      until now did not exist
- [ ] 8.6 Note in `IDEAS.md` that `rune-bg2.gif` was settled against, so the
      question `d2-theme` left open does not get reopened by inspection

## 9. Acceptance

- [ ] 9.1 Add no dependency. `clsx` and `class-variance-authority` cover the
...
- [ ] 9.3 Confirm no component carries a literal colour value, and that the four
      new tokens are the only ones added
- [ ] 9.4 Run the full local gate — `pnpm typecheck`, `pnpm lint`,
      `pnpm format:check`, `pnpm test`, `pnpm build` — and confirm all five exit
      zero
- [ ] 9.5 **Look at it.** `pnpm dev`, then check: rune icons legible at row size,
...
- [ ] 9.6 Check the keyboard path with no pointer at all: Tab to a name, Space to
      open, Tab within the dialog and confirm focus does not reach a row behind it,
      Escape to close, focus back on the name
- [ ] 9.7 Confirm `Fortitude`'s 26 property lines and every emphasised value read
      exactly as the dataset holds them — the round-trip test proves the characters,
      not that the right ones are emphasised
- [ ] 9.8 Run `openspec validate --changes runeword-table --strict`
- [ ] 9.9 Commit as `feat(table): add read-only runeword table`
