...
title's own line beside the feedback link, and its panel SHALL open beneath both
at the page's own measure — which is what makes the control a button reporting its
expanded state rather than the native disclosure element used elsewhere in the
project, since that element requires its control to be its own first child and
therefore cannot straddle two rows of the header. The control SHALL report
whether it is expanded and SHALL name the element it controls. The panel SHALL
remain in the document while closed, and SHALL be hidden by a means that holds
even where the stylesheet has not loaded. Nothing about the disclosure SHALL be
persisted. Every word of it SHALL resolve through the display-copy layer.
...
- **THEN** it states what the list is and how the page is used — marking a
  ...
  the two remaining panels, where a runeword's granted properties are, and where
  the reader's progress is kept
