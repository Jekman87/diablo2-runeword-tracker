# localised-dataset-text Specification

## Purpose

How the dataset's own text — runeword names, rune names, base item categories,
item-type restrictions, property lines and notes — reaches the reader in the
active language: that a translated record shows strictly one language on screen
with not one Latin letter in Russian, that a record without a Russian variant
falls back to English as a whole rather than field by field, that a single
locale projection is what rendering, search matching and sort comparison all
read, and that localisation stays presentation only — canonical English names
remain the identifiers behind stored progress, sprite lookup, the slot mapping
and export.
