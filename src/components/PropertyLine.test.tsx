...
const allLines = runewords.flatMap((runeword) => runeword.properties);

describe("the round-trip over the whole dataset", () => {
  it("holds 975 property lines to check", () => {
    expect(allLines).toHaveLength(975);
  });

  it("reproduces every line exactly from its rendered fragments", () => {
    // One render for all 975, so the assertion is over what the browser would
...
describe("which fragments are emphasised", () => {
  ...
  it("leaves all 66 digitless lines unemphasised", () => {
    const digitless = allLines.filter((line) => !/\d/.test(line));

    expect(digitless).toHaveLength(66);
