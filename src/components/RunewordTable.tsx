 ...
 *
 * It holds no state of its own at all now. It used to own which runeword was
 * selected and the one `<dialog>` every row shared; a panel that opens on hover
 * has to be positioned against the name the pointer is over, and `useHover`
 * binds to a single reference element, so each row owns its own panel. Only the
 * open one is ever rendered, which is what keeps that from becoming 99 panels of
 * markup.
 */
